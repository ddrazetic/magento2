...
require([
    ...
    'use strict';
    console.log('loggiran: ' + commentsData.isLoggedin)

    var loginUrl = commentsData.loginUrl,
        like = commentsData.like,
        reply = commentsData.reply,
        isLogged = commentsData.isLoggedin,
        likedColor = commentsData.likedColor,
        messengerBox = {
            cmt_warning: commentsData.cmt_warning,
            exist_email_warning: commentsData.exist_email_warning,
            login_warning: commentsData.login_warning,
            comment_approve: commentsData.comment_approve
        };
    ...
    function removeErrorMessages(validationName, validationEmail) {
        let name = $("#default-cmt__content__cmt-block__guest-box__name-input");
        let nameBoxError = $("#default-cmt__content__cmt-block__guest-box__name-input-error");
        let email = $("#default-cmt__content__cmt-block__guest-box__email-input");
        let emailBoxError = $("#default-cmt__content__cmt-block__guest-box__email-input-error");
        if (validationName) {
            name.removeClass('mage-error')
            nameBoxError.text("")
        }
        if (validationEmail) {
            email.removeClass('mage-error')
            emailBoxError.text("")
        }
    }
    ...
    function checkGuestName() {
        ...
        if (name.val().length < 1) {
            name.addClass('mage-error')
            nameBoxError.text("This is a required field.")
    ...
    function checkGuestEmail() {
        ...
        if (email.val().length < 1) {
            ...
            emailBoxError.text("This is a required field.")
            // console.log('required email!')
             return false
        }
        else if(!email.val().match(mailformat)){
            email.addClass('mage-error')
            emailBoxError.text("Please enter a valid email address (Ex: johndoe@domain.com).")
            // console.log('required email!')
            return false
        }
    ...
    function checkGuestFormValidate() {
        if (isLogged == 'No') {
            ...
            const validationEmail = checkGuestEmail();
            const validationName = checkGuestName();
            const validation = validationName && validationEmail;
            removeErrorMessages(validationName, validationEmail)
            // console.log(validation)
            // console.log('name: ' + $("#default-cmt__content__cmt-block__guest-box__name-input").val())
            // console.log('email: ' + $("#default-cmt__content__cmt-block__guest-box__email-input").val())
            return validation
        }
        return true;
    }
